...
SnakeGame::SnakeGame(string path){
    choice = "";
    frameCount = 0;
    initialize_game(path);
}

void SnakeGame::initialize_game(string path){
    //carrega o nivel ou os níveis
    ifstream levelFile(path); //só dá certo se o jogo for executado dentro da raíz do diretório (vc vai resolver esse problema pegando o arquivo da linha de comando)
    int lineCount = 0;
    string line;
    if(levelFile.is_open()){
        ...
        nivel.create_map(maze);
        IA.find_solution(maze, nivel.init_position(), nivel.fruit_position());
    }
    state = RUNNING;
}
...
void SnakeGame::render(){
    ...
    switch(state){
        case RUNNING:
            ...
            nivel.print_map();
            if(nivel.update_map(IA.next_move())){
                IA.find_solution(maze, nivel.init_position(), nivel.fruit_position());
            }
            break;
        case WAITING_USER:
